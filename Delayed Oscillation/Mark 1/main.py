...
def dV(phi,p):
    ...
    dV_norm = p * (1 + ((phi) ** 2) / p) ** (-p - 1) * (2 * phi / p)
...
def H(args, p):
    [dphi, phi] = args
    H = f*sqrt(0.5*absolute(dphi)**2+V(phi,p))/(sqrt(V(phi_i,p)))
...
def ODE_sys(t,u,arg):
    ...
    sys = [
        ...
        dA, -H_i*dA-((k/a(t,p))**2-(h*alpha*k/a(t,p)*dphi))
...
def ODE_solver(arg):
    ...
    return np.array([t,u_0,u_1,u_2,u_3])


def Phi_Spooler(args):

    p = args

    stack = []
    for p_i in args:
        sol = Scalar_solver(p_i)
        stack.append(sol)

    return np.array(stack)

def a_spooler(p, Phi_stack):

    a_stack = []
    for idx_p in np.arange(0,len(p),1):
        p_i = p[idx_p]
        substack = Phi_stack[idx_p]
    ...
    return np.array(a_stack)

def initiation_matrix(args):
    [p, Phi_stack, use_limit_of, lim_val] = args
    trigger_t = []
    for idx_p, item_p in enumerate(p):
        stack = Phi_stack[idx_p]
...
def kill_matrix(args):
    [p, Phi_stack, splice, use_limit_of, lim_val] = args
    splice = int(splice)
    kill_t = []
    for idx_p, item_p in enumerate(p):
        stack = Phi_stack[idx_p]
...
def Gauge_ODE(t, u, arg):
    ...
    [dphi, phi] = [
        mapper(t, Phi_stack[idx_p, 0], Phi_stack[idx_p, 2]),
        mapper(t, Phi_stack[idx_p, 0], Phi_stack[idx_p, 1])
    ]
...
def Gauge_solver(item_k,*,  arg, a_stack, Phi_stack):
    [idx_p,h,alpha,p, t_f, t_i] = arg

    ode_para = [idx_p, h, alpha, item_k,p, a_stack, Phi_stack]
...
def Gauge_spooler(p,alpha,k,a_stack, Phi_stack, kill_matrix,trigger_mattrix, Master_path):
    ...
    for idx_p in np.arange(0,len(p), 1):
        ...
        for h_i in h:
            ...
            for alpha_i in alpha:
                ...
                arg = [idx_p, h_i, alpha_i, p_i, t_f, t_i]

                pool = Pool(cpu_count())
                par = functools.partial(Gauge_solver,
                                        arg=arg, a_stack=a_stack, Phi_stack=Phi_stack)

                sol_k = np.array(pool.map(par, k[idx_p]))
                pool.close()

                name = 'alpha:%.5f_h:%s_p:%.3f'%(alpha_i,h_i,p_i)
                save(sol_k,name, 'Raw', Master_path=Master_path)

                timer('Finished stack alpha:%.5f_h:%s_p:%.3f:'%(alpha_i,h_i,p_i), start)

    return

def Asymptotes(p,alpha,k,a_stack, Phi_stack, Master_path):
...
def Shredder_B(args ,Gauge_stack):
    ...
    a_x = mapper(x, a_stack[idx_p, 0], a_stack[idx_p, 1])
...
def Rho_B_spooler(p,alpha, k, x_len, a_stack, Master_path):
        ...
        for idx_alpha, item_alpha in enumerate(alpha):
            ...
            for idx_h, item_h in enumerate(h):
                ...
                for x in np.linspace(t_i, t_f, x_len):
                    Shredder_args = [x,k[idx_p], a_stack, idx_p]
...
def Rho_E_spooler(p,alpha, k, x_len, a_stack, Master_path):
...
def Shredder_helicity(args, Master_path):
    [x, k, a_stack,idx_p, item_p, item_alpha] = args

    h = [-1,1]
    name_list = ['alpha:%.5f_h:%s_p:%.3f' % (item_alpha, item_h, item_p) for item_h in h]
...
def Helicity_spooler(p,alpha, k, x_len, a_stack, Master_path):
    h = [-1,1]
    stack_p = []
    for idx_p, item_p in enumerate(p):
        stack_alpha = []
        for idx_alpha, item_alpha in enumerate(alpha):
            stack_x = []
            for x in np.linspace(t_i, t_f, x_len):
                Shredder_args = [x, k[idx_p], a_stack, idx_p, item_p, item_alpha]
                Hel = Shredder_helicity(Shredder_args, Master_path)

                stack_x.append(Hel)
            print(np.shape(stack_x))
            name = 'alpha:%.5f_p:%.3f' % (item_alpha, item_p)
