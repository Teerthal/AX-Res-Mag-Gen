...
f_list = np.array([0.1, 0.05, 0.01, 0.0075, 0.005, 0.0025, 0.001, 0.0005, 0.00025, 0.0001])
...
def Scalar_plot():
    ...
    analytical_eps = np.array([*map(analytical_epsilon, N)])
...
delta = np.linspace(2., 12., delta_intervals)
...
def Asymptotic_plot():
    ...
    index_b = [-1]
    index_alpha = [0]
    plt.plot([],[], label=r'$\mathcal{A}_-$', linestyle='-', color='k')
    plt.plot([],[], label=r'$\frac{d\mathcal{A}_-}{dN}$', linestyle='-.', color='k')
    for j in index_f:
        f_j = f_list[j]
        b = b_list(f_j)
        alpha = alpha_list(f_j)
        ...
        for i in index_b:
            ...
            for z in index_alpha:
                ...
                plt.loglog(k_map / a(N_final), stack[1], linestyle = '-')
                plt.loglog(k_map / a(N_final), stack[2], linestyle = '-.')
                plt.legend()
...
def Lambda_End(alpha_z, b_i, f_j):
    stack = load('alpha:%.4f_b:%.4f_f:%.5f' % (alpha_z, b_i, f_j), 'Asymptotes')

    A_end = stack[1]
    Integrand_num = (2 * pi) ** 2 * k_map ** 2 / 2 * (A_end ** 2)
    num = trapz(Integrand_num, x=k_map)
    Integrand_den = pi * k_map ** 3 * (A_end ** 2)
    den = trapz(Integrand_den, x=k_map)
...
def Magnetic_Field(alpha, f, b):
    ...
    Gauss = 6.8e-20
    Mpc = 1e-39*6.4
    B_max_final = (1+z_rec)**(-2/3)*T_0**(7/3)*B_max**(2/3) * (T_r/M_PL)**(1/3)*(H_f)**(1/3)*M_PL**2*Gauss
    #Lamda_max_final = (1+z_rec)**(-2/3)*T_0**(7/3)*B_max**(2/3) * (T_r/M_PL)**(1/3)*(H_f)**(1/3)*Mpc/M_PL
    lamda = Lambda_End(alpha, b, f)*a(-70)*Mpc
    print(B_max_final, lamda)
    return np.array([B_max_final, lamda])
