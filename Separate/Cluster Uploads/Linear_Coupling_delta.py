...
def b_list(item_f):
    b = np.linspace(0.075,0.5,b_list_size)/item_f
    return b

def alpha_list(item_f):
    alpha = np.linspace(15,50, alpha_list_size)*item_f
...
f_list = np.geomspace(0.01, 0.01, 1)
        ...
        for i in index_b:
            ...
            for z in index_alpha:
                ...
                for h in [-1]:
                    ...
                    stack_h.append(core(delta, h, alpha, i, j))
...
if __name__ == '__main__':
    stack = np.array(pool.map(execute, delta))
    pool.close()
    ...
    for j in index_f:
        ...
        for i in index_b:
            ...
            for z in index_alpha:
                ...
                asymptotes = np.array([delta, stack[:, 0, j, i, z, 0, 1, -1], stack[:, 0, j, i, z, 0, 2, -1]])
...
def Backreaction_Functional(N_index, index_b, index_f, index_alpha):

    Integrand = k_map**2/(16*pi**2) * (A(N_index, index_b, index_f, index_alpha) ** 2)
    return trapz(Integrand, x=k_map)
...
def Spooler_Bacreaction(index_b, index_f, index_alpha):
    ...
    for j in index_f:
        ...
        for i in index_b:
            ...
            for z in index_alpha:
                ...
                if __name__ == '__main__':
                    Backreaction_map = pool.map(functools.partial(Backreaction_Functional, index_b=i, index_f=j, index_alpha=z), N_index)
                    pool.close()
                ...
                #Backreaction = H_map[:-1]*alpha_z/f_j*dy/dN
                Backreaction = dy/dN

                stack_z.append(Backreaction)
            stack_i.append(stack_z)
        stack_j.append(stack_i)
...
Backreaction_Spool = np.array([Spooler_Bacreaction(index_b,index_f, index_alpha)])
print('Shape of Backreaction Spool:', np.shape(Backreaction_Spool) )

save(Backreaction_Spool, 'Backreaction')
    ...
    for j in index_f:
        ...
        for i in index_b:
            ...
            for z in index_alpha:
                ...
                plt.semilogy(N[:-1], Backreaction_Spool[0, j, i, z] , label = r'$\frac{\alpha}{f}< E.B >$')
