    ...
    for l in [0,1]:
        ...
        for j in index_b:
            ...
            for z in [10,-1]:
                ...
                gauge_sr = load('slow_roll_alpha_f:%.4f'%(item_alpha/f_list[0]), 'Raw')
                #plt.loglog(k_map, gauge_sr[0,:,1,-1], label='s bf:%.4f' %(item_b*item_f));plt.loglog(k_map,stack[0,:,1,-1]);plt.show()
                for i in np.round(np.linspace(0, len(k_map), 3)).astype(int):

                    ax1.semilogy(gauge_sr[0, i, 0], gauge_sr[0, i, 1], linestyle=':', linewidth = 5., alpha = 0.25)

                    k_i = k_map[i]
                    ax1.set_ylabel(r'$|\mathcal{A}|$')
                    ax1.semilogy(stack[0, i, 0], stack[0, i, 1], label=r'$k/a_f:%s ,bf:%s$'
                                                                       % (k_i/a(stack[0,i,0][-1]), item_b * item_f))
                    plt.ylabel(r'$|\frac{d\mathcal{A}}{dx}|$')

                    phi_N = remapped_scalar[j, i, :, 2]
                    dphi = remapped_scalar[j, i, :, 1]
                    ax2.semilogy(phi_N, abs(dphi))
...
rewrite_Asymptote = 'F'#input('Rewrite Asymptote data directory(T/F):')
