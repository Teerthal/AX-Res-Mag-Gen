...
f_list = np.array([1e-2,1e-3,1e-4])
####################################
    ...
    for l in [0]:
        ...
        for j in [-1]:#index_b:
            ...
            for z in [-1]:
                ...
                N_start = stack[0, -1, 0][0]
                k_idxs = np.round(np.linspace(800, len(k_map) - 1, 2)).astype(int)
                colors_k = cm.rainbow(np.linspace(0,1,len(k_idxs)))

                phi_N = remapped_scalar[l, j, :, 2]
                dphi = remapped_scalar[l, j, :, 1]
                a_map = np.array([a_corr(N, l, j) for N in phi_N])
                k_map = np.array([k(delt, l, j) for delt in delta])
                print(phi_N[-1])
                for cnt_i, i in enumerate(k_idxs):
                    #print(delta[i])
                    #print(i)
                    ...
                    ax1.semilogy(gauge_sr[0, i, 0], gauge_sr[0, i, 1],
                                 color=colors_k[cnt_i], linewidth = 4., alpha = 0.4)
                    ax1.semilogy(gauge_sr[1, i, 0], gauge_sr[1, i, 1], color=colors_k[cnt_i],
                                 linestyle='--' , linewidth = 4., alpha = 0.4)

                    ax1.semilogy([N_start,gauge_sr[0,i,0][0]], [1, 1], color=colors_k[cnt_i], linewidth = 4., alpha = 0.4)
                    ax1.semilogy([N_start, gauge_sr[1,i,0][0]], [1, 1], color=colors_k[cnt_i], linewidth = 4., alpha = 0.4, linestyle='--')

                    k_i = k_map[i]
                    ax1.set_ylabel(r'$|\sqrt{2k}\mathcal{A}_\pm|$')
                    ax1.semilogy(stack[0, i, 0], stack[0, i, 1], color=colors_k[cnt_i],linewidth = 1.5)  # , label=r'$k/a_f:%s ,bf:%s$'% (k_i/a(stack[0,i,0][-1]), item_b * item_f))
                    ax1.semilogy(stack[1, i, 0], stack[1, i, 1], linestyle='--', color=colors_k[cnt_i],
                                 label=r'$k/a_f:%.2f \quad bf:%.2f$' % (k_i / a(stack[0, i, 0][-1]), item_b * item_f),linewidth = 1.5)
                    # ax1.set_ylabel(r'$|\frac{d\mathcal{A}}{dx}|$')
                    # plt.legend()

                    ax1.semilogy([N_start, stack[0,i,0][0]], [1, 1], color=colors_k[cnt_i],linewidth = 1.5)
                    ax1.semilogy([N_start, stack[1,i,0][0]], [1, 1], color=colors_k[cnt_i], linestyle='--',linewidth = 1.5)

                    ax2.plot(phi_N, item_alpha/item_f*abs(dphi),
                             linewidth=1.)  # , label = r'$f:%.4f \quad  bf:%.2f$'%(item_f, item_f*item_b))
                    phi_sr_N = remapped_scalar_sr[:, 2]
                    dphi_sr = remapped_scalar_sr[:, 1]
                    print('sr',phi_sr_N[-1])
                    ax2.plot(phi_sr_N, item_alpha/item_f*abs(dphi_sr), color='grey',linewidth = 1.5)# , linewidth = 3, alpha = 0.25)

                    ax2.plot(phi_N, k_map[i]/(a_map*H_initial),color=colors_k[cnt_i], linestyle=':',linewidth = 1.5)
...
def Asymptotic_plot():
    for j in [0]:
        ...
        for z_idx, z in enumerate([1,-1]):#index_alpha:
            alpha_z = alpha[z]
            alpha_sr_z = alpha_sr[z]
            print(alpha_z/f_j)
            ...
            plt.subplot(211)
            plt.loglog(stack_sr[0], absolute(stack_sr[1]-A_zero_sr), color = 'grey', linewidth = 5, alpha = 0.35)
            dphi_sr = remapped_scalar_sr[:, 1]; xi=absolute(dphi_sr*alpha_sr_z/f_j)
            N_sr = remapped_scalar_sr[:, 2]
            A_ana = []
            a_map = np.array([*map(a_corr_sr, N_sr)])

            for k_a in stack_sr[0]:

                xi_cross = np.argmin(abs(xi*k_a/(a_map*H_initial)-(k_a/(a_map*H_initial))**2))
                A_ana.append(exp(pi*xi[xi_cross])/(2*pi*xi[xi_cross]))

            plt.loglog(stack_sr[0], A_ana,color=colors_alpha[z_idx], linestyle='--')

            for i in [0]:#index_b:
                b_i = b[i]

                stack = load('alpha_f:%.4f_b:%.4f_f:%.5f' % (alpha_z/f_j, b_i, f_j), 'Asymptotes')
                #max_A = np.argmax(absolute(stack[1])); k_max = stack[0][max_A]; print(k_max,log(k_max))
                stack_0 = load('alpha_f:%.4f_b:%.4f_f:%.5f' % (0./f_j, b_i, f_j), 'Asymptotes')
                A_zero, del_A_zero = [stack_0[1], stack_0[2]]
                plt.subplot(211)
                plt.loglog(stack[0], absolute(stack[1]-A_zero), linestyle = ['-','--'][i], color = colors_alpha[z_idx])#,label=r'f:%.4f  bf:%.2f' % (b_i * f_j))
                N_kill = Buffer[j, i, 2][-1]
                plt.subplot(212)
                plt.loglog(stack[0], absolute(stack[2]-del_A_zero), linestyle = ['-','--'][i], color = colors_alpha[z_idx])
...
def Shredder_Back(A_z, k_map):
    #A_z = stack[0, :, 3, N_index]
    Integrand = k_map ** 2 / (8 * pi ** 2) * absolute(A_z)**2
...
def Spooler_rho_B():
    ...
    for j in index_f:
        ...
        for i in index_b:
            ...
            targ_alpha_idxs = [np.argmin(abs(targ - alpha)) for targ in targ_alpha*item_f]
            ...
            for z in targ_alpha_idxs:
                ...
                for item_N in intg_N:
                    A_z = [gen_map(item_N, stack[0, i, 0], stack[0, i, 3])
                           for i in np.arange(0, N_start_intervals,1)]

                    Shred = Shredder_B(sqrt(np.array(A_z)), k_map)
    ...
    for j in index_f:
        ...
        for i in index_b:
            ...
            for z in targ_alpha_idxs:
                ...
                for item_N in intg_N:
                    del_A = [gen_map(item_N, stack[0, i, 0], stack[0, i, 3])
                           for i in np.arange(0, N_start_intervals, 1)]

                    Shred = Shredder_E(sqrt(np.array(del_A)), k_map)
                    rho_E_map.append(Shred)
        ...
        for i in index_b:
            ...
            for z in targ_alpha_idxs:
                ...
                for item_N in intg_N:
                    ...
                    Shred = Shredder_Back(sqrt(np.array(A_z)), k_map)
                    Backreaction_map.append(Shred)
...
def Spooler_rho_B_sr():
    ...
    targ_alpha_idxs = [np.argmin(abs(targ - alpha_sr)) for targ in targ_alpha * f_list[0]]
...
rewrite_rho_B_sr = 'F'#input('Rewrite Rho_B SR file:')
...
rewrite_rho_E_sr = 'F'#input('Rewrite Rho_E SR file:')
...
rewrite_Backreaction_sr = 'F'#input('Rewrite Backreaction SR file:')
...
def rho_Back_plot():
    ...
    for idx_f, j in enumerate(index_f):
        ...
        colors = cm.rainbow(np.linspace(0, 1, (len(index_b)+1) * len(targ_alpha)))
        color_idx = 0
        plt.subplot(2, 2, 1 + idx_f)
        ...
        for idx_b, i in enumerate(index_b):
            ...
            for z in targ_alpha_idxs:
                ...
                rho_B_sr = abs(rho_B_sr[:, norm])# - rho_B_0[:, norm]) / a_map ** 4
                rho_E_sr = abs(rho_E_sr[:, norm])# - rho_E_0[:, norm]) / a_map ** 2
                ...
                if idx_f+1 == 2:
                    ...
                    label = None
                plt.semilogy(intg_N, (rho_B) / rho_phi, linestyle='-', color = colors[color_idx], label=label)
                ...
                plt.semilogy(intg_N, (rho_B_sr) / rho_phi, linestyle='-', color=colors[color_idx])

                plt.semilogy(intg_N, Bck_sr, color=colors[color_idx], linewidth=0.75, linestyle='--')

                plt.semilogy(intg_N, rho_E_sr / rho_phi, linestyle=':',
                             color=colors[color_idx])
