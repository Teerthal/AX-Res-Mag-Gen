...
def Scalar_plot():
    #plt.subplot(211)
    plt.plot(slow_roll_stack[2]-slow_roll_stack[2][-1], slow_roll_stack[1]**2/2,label='slow roll')
    plt.plot([N_initial - N_final+2, 0], [1, 1])
    ...
    for j in index_f:
        ...
        for i in index_b:
            ...
            plt.plot(N_phi_solved - N_phi_solved[-1], phi_prime_solved ** 2 / 2, label='bf:%.4f'%(b_i*f_j) )
...
def Gauge_eps_curves():
    ...
    for l in [0]:
        ...
        colors_b = cm.rainbow(np.linspace(0.05,1,len(index_b)))
        for j, idx_b in enumerate(index_b):
            item_b = b[j]
            
            k_map = np.array([*map(functools.partial(k, index_f=l, index_b=j),delta)])

            for z in [np.argmin(abs(alpha/item_f-9))]:
                item_alpha = alpha[z]
                print(item_alpha/item_f)
                stack = load('alpha_f:%.4f_b:%.4f_f:%.5f' % (item_alpha/item_f, item_b, item_f), 'Raw')
                gauge_sr = load('slow_roll_alpha_f:%.4f'%(item_alpha/f_list[0]), 'Raw')
                #plt.loglog(k_map, gauge_sr[0,:,1,-1], label='s bf:%.4f' %(item_b*item_f));plt.loglog(k_map,stack[0,:,1,-1]);plt.show()
                for i in np.round(np.linspace(750, 1000, 1)).astype(int):
                    print(i)
                    
                    ax1.semilogy(gauge_sr[0, i, 0], gauge_sr[0, i, 1], color = 'red')#, linewidth = 3., alpha = 0.25, )
                    ax1.semilogy(gauge_sr[1, i, 0], gauge_sr[1, i, 1], color = 'red', linestyle = '--')#, linewidth = 3., alpha = 0.25, )
                    k_i = k_map[i]
                    ax1.set_ylabel(r'$|\sqrt{2k}\mathcal{A}_\pm|$')
                    ax1.semilogy(stack[0, i, 0], stack[0, i, 1], color = colors_b[j])#, label=r'$k/a_f:%s ,bf:%s$'% (k_i/a(stack[0,i,0][-1]), item_b * item_f))
                    ax1.semilogy(stack[1, i, 0], stack[1, i, 1], linestyle = '--', color = colors_b[j],label=r'$k/a_f:%.2f \quad bf:%.2f$'%(k_i/a(stack[0,i,0][-1]), item_b * item_f))
                    ...
                    ax2.plot(phi_sr_N, abs(dphi_sr), color = 'red')#, linewidth = 5, alpha = 0.25)
...
def Asymptotic_plot():
    for j in [0]:
        ...
        targ_alpha_idxs = [5, 20, 35, -1]
        colors_alpha = cm.rainbow(np.linspace(0, 1, len(targ_alpha_idxs)))
        for z_idx, z in enumerate(targ_alpha_idxs):#index_alpha:
            alpha_z = alpha[z]
            alpha_sr_z = alpha_sr[z]
            print(alpha_z/f_j)
            ...
            #plt.subplot(211)
            plt.loglog(stack_sr[0], absolute(stack_sr[1]-A_zero_sr), color = 'grey', linewidth = 5, alpha = 0.45)
    ...
    plt.xlabel(r'$k(a_{f}H_{f})^{-1}$')
    plt.ylabel(r'$\sqrt{2k}|\mathcal{A}_\pm(k,N_{f})|$')
    return plt.show()
    ...
    for idx_f, j in enumerate(index_f):
        ...
        for idx_b, i in enumerate(index_b):
            ...
            for z in np.arange(0,len(alpha), 1):
                ...
                stack_z.append([alpha_z/f_j, rho_B[-1], rho_E[-1], Bck[-1]])

            stack_z = np.array(stack_z)
            print(np.shape(stack_z))
            plt.plot(stack_z[:,0], stack_z[:,1]/rho_phi, marker='+', color=colors[color_idx])

            plt.semilogy(stack_z[:,0], stack_z[:,3]/dVdphi, color=colors[color_idx], linewidth=0.75, linestyle='--')

            plt.semilogy(stack_z[:,0], stack_z[:,2] / rho_phi, marker='.',color=colors[color_idx])
            #plt.yscale('Log')
            color_idx = color_idx + 1
...
def B_alpha():
    ...
    fig, axes = plt.subplots(1, len(f_list))
    ...
    for j, item_f in enumerate(index_f):
        ...
        for i, item_b in enumerate(b):  # index_b:
            ...
            stack_z = []
            for z_idx, z in enumerate(np.arange(1,len(alpha), 1)):
                alpha_z = alpha[z]
                alpha_sr_z = alpha_sr[z]
                print(alpha_z / f_j)
                stack_sr = load('slow_roll_alpha_f:%.4f' % (alpha_sr_z / f_list[0]), 'Asymptotes')
                ...
                Lambda_f = 1 / (max_k * H_inf * Mpl)  ## dimensionless, normalized by GeV^-1
                B_k_f = sqrt(H_inf ** 4) * (max_k ** 4) * (max_A ** 2) * (Mpl ** 2)  ##Dimensionless, normalized by GeV^2
                a_R = T_0 / (T_R * Mpl)  ## a_rT_r = a_0T_r
                #a_R = 1e-29 ##Subhramanian entropy conservation
                Lambda_0 = 3.3e5 * a_R * (Lambda_f) ** (1 / 3) * (B_k_f) ** (2 / 3)
                B_0 = 1e-8 * Lambda_0

                stack_z.append([alpha_z/f_j, Lambda_0, B_0])

            slit = np.array(stack_z)
            ax1.semilogy(slit[:,0], slit[:,2], linestyle = b_styles[i])
