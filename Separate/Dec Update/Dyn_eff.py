...
def core(delta, h, alpha, index_b, index_f):
    ...
    N_start = N_kill - N_gauge_off - delta - 10.
    ...
    for l in index_f:
        ...
        for j in index_b:
            ...
            for z in index_alpha:
                ...
                for i in [-1]:#np.arange(0, delta_intervals, 1):
                    plt.subplot(211)
                    ...
                    A_prime = (stack[0, i, 1])
                    a_map = np.array([*map(functools.partial(a_corr, index_f=l, index_b=j), stack[0,i,0])])
                    plt.semilogy(stack[0, i, 0], A_prime/a_map*k_map[i], label='k:%s' % k_i)
                    plt.legend()
        plt.show()
    ...
    for j in index_f:
        ...
        for i in index_b:
            ...
            for z in index_alpha:
                ...
                plt.semilogy(intg_N, abs(rho_B_Spool)/a_map**4, label=r'$\rho_B$')
                plt.semilogy(intg_N, abs(rho_E_Spool)/a_map**2 , label=r'$\rho_E$')
                plt.legend()
