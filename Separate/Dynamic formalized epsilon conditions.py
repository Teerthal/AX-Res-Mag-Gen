...
Guide_Buffer = np.array([Phi_Solver(f_list)])
...
kill_matrix = kill_eps_limit(); print('shape of epsilon kll matrix', np.shape(kill_matrix))

def Scalar_Core_Pri(b, f, eps_limit):
    Parameters = np.array([b, f])
    solver = ode(Scalar_ODE).set_integrator('vode').set_f_params(Parameters)
    Scalar_init = np.array([phi(N_initial), phi_prime(N_initial)])
    solver.set_initial_value(Scalar_init, N_initial)
...
Buffer = np.array([Phi_Solver_Pri(f_list)])
...
def N_k(delta, index_f, index_b):
    kill_time = Buffer[0, index_f, index_b, 2][-1]
    N_kill = kill_time
    #N_k = delta + N_start
    N_k = N_kill - delta
    return N_k
...
def a_corr(N, index_f, index_b):
    N_phi_solved = Buffer[0, index_f, index_b, 2]
        ...
        for z in index_alpha:
            ...
            for i in index_b:
                ...
                for l in delta:
                    ...
                    #plt.plot(gauge_N, k_sr(l)/(a_full*H_full))
...
def core(delta, h, alpha, index_b, index_f):
    ...
    N_kill =Buffer[0, index_f, index_b, 2][-1]#;print('kill time', N_kill)
...
def Shredder_B(stack, k_map, N_index):
...
print('Starting slow roll calculations')
...
def Spooler_rho_B_sr():
    ...
    for z in index_alpha:
        ...
        stack = load('slow_roll_alpha:%.4f' % alpha_sr[z], 'Raw')
        ...
        # pool.close()
        print(np.shape(rho_B_map))
        save(rho_B_map / (a_map_Gauge * H_map_Gauge) ** 4, 'slow_roll_alpha:%.4f' % alpha_sr[z], 'Rho_B')
...
def Spooler_rho_E_sr():
    ...
    for z in index_alpha:
        ...
        save(rho_E_map / (a_map_Gauge * H_map_Gauge) ** 4, 'slow_roll_alpha:%.4f' % alpha_sr[z], 'Rho_E')
    return
...
def Spooler_Bacreaction_sr():
    ...
    for z in index_alpha:
        ...
        save(Backreaction_map, 'slow_roll_alpha:%.4f' % alpha_sr[z], 'Backreaction')
    return
    ...
    for j in index_f:
        ...
        for i in index_b:
            ...
            for z in index_alpha:
                ...
                gauge_stack = load('alpha:%.4f_b:%.4f_f:%.5f' % (alpha_z, b_i, f_j), 'Raw')
                N = gauge_stack[0, i, 0]
