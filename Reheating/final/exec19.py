...
alpha = np.array([0, 1.4,1.6,1.8,2,2.2,2.4,2.6,2.8,3])
len_k = 24*60
x_len = steps/10       #Integration steps for computing energy density, helicity and backreaction
##Kill Conditions##
splice = steps/100
...
f = np.geomspace(1e-1,5e-2, 2)
#f = np.array([5e-2, 1e-2, 5e-3])
...
Master_path = "/media/teerthal/Repo 2/Reheating/Set_19"
...
#load_or_exec_multi_file(Master_path, 'Backreaction', BR_spooler, Rho_E_spool_args)
...
#exit()
#rho_plot(p, alpha, k, x_len, a_stack, Master_path, f, trigger_matrix, kill_arr, Phi_stack, asymp_stack)
#rho_plot_spec(p, alpha, k, x_len, a_stack, Master_path, f, trigger_matrix, kill_arr, Phi_stack)
...
#B_present([p, f, alpha, k, kill_arr, Phi_stack, Master_path, k_generator, a_stack], asymp_stack)


asymp_plot([p,alpha,k,a_stack, Phi_stack, f, kill_arr], asymp_stack)

rho_hel_plot(p, alpha, k, x_len, a_stack, Master_path, f, trigger_matrix, kill_arr, Phi_stack, asymp_stack)
rho_B_E_hel_plot(p, alpha, k, x_len, a_stack, Master_path, f, trigger_matrix, kill_arr, Phi_stack, asymp_stack)
