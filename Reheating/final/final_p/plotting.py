...
def scalar_plot(p, Phi_Stack, a_stack, kill_arr, trigger_arr, k, f):
    ...
    colors = cm.rainbow(np.linspace(0, 1, len(p)*len(f)+1))
    color_cnt = 0

    ax0 = plt.subplot2grid((2, 4), (0, 0), colspan=3)
    ax1 = plt.subplot2grid((2, 4), (0, 3), colspan=1)
    ax2 = plt.subplot2grid((2, 4), (1, 0), colspan=2, sharex=ax0)
    ax3 = plt.subplot2grid((2, 4), (1, 2), colspan=2)

    for cnt_f, idx_f in enumerate(range(len(f))):
        item_f = f[idx_f]
        for idx_p in np.arange(0,len(p),1):
            ...
            ax0.loglog(t, abs(phi), label=r'$p:%.1f f:%.3f \quad H_{osc}/m:%.3f$' % (p_i,item_f, H_osc), color = colors[color_cnt])
        ...
        for idx_p, item_p in enumerate(p):
            ...
            for idx_h, item_h in enumerate(h):
                ...
                for cnt_alpha, idx_alpha in enumerate([-1]):
                    ...
                    plt_k_idxs = range(len(k[idx_f][idx_p]))
            ...
            ax2.set_xlabel(r'$\tilde{t}$');ax1.set_title(r'$p:%.1f f/{\rm Mpl}:%.3f$'%(item_p,item_f))
...
def asymp_plot(args, stack):
    ...
    #targ_alpha = [0.5,0.6,0.7, 0.8]; targ_alpha = alpha
...
def rho_hel_plot(p, alpha, k, x_len, a_stack, Master_path, f, trigger_matrix, kill_matrix, Phi_Stack, asymp_stack):
    ...
    H_inf = 1e-8
    targ_f = [0.05, 0.01, 0.005]; targ_f = f

    p_idxs = [0]
    targ_alpha = [0.25,0.5,1, 1.4]   ; targ_alpha = np.array(alpha)
    # alpha_idxs=range(len(alpha))
    alpha_idxs = [np.argmin(abs(targ - alpha)) for targ in targ_alpha]
    f_idxs = [np.argmin(abs(targ - f)) for targ in targ_f]
    colors = cm.rainbow(np.linspace(0, 1, len(alpha_idxs)*len(p_idxs)));
    ...
    for cnt_f, idx_f in enumerate(f_idxs):
        ...
        for cnt_p, idx_p, in enumerate(p_idxs):
            ...
            for cnt_alpha, idx_alpha in enumerate(alpha_idxs):
                ...
                ax0.plot([],[], label = r'$p:%.1f \alpha:%.2f$'%(item_p, item_alpha))
                color_cnt = color_cnt + 1
