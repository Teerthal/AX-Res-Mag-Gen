...
def hel_spooler_2(p, alpha, k_stack, a_stack, Phi_stack, Master_path, f, x_len, trigger_matrix, kill_matrix):
    ...
    for idx_f, item_f in enumerate(f):
        for idx_p, item_p in enumerate(p):
            ...
            intg_k = range(len(k))
            for cnt_alpha, idx_alpha in enumerate(range(len(alpha))):
                ...
                pool = Pool(cpu_count())
                par_1 = functools.partial(intg_par, A_h = A_h, intg_k=k[intg_k])

                Integrand = np.array(pool.map(par_1, range(int(x_len))))
                pool.close()
                
                pool = Pool(cpu_count())
                par_2 = functools.partial(intg_par_2, A_h = A_h, intg_k=k[intg_k])
