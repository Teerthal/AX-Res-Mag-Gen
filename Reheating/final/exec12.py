...
alpha = np.array([0.,0.5,0.75, 1.,1.25])
len_k = 24*30
...
Data_set = 'p_%.2f_%.2f_%.1f_f_%.4f_%.4f_%.1f_k__%.1f_steps_%.1f'%(p[0], p[-1], len(p),
                                                                     f[0], f[-1], len(alpha), len_k, steps)

#Master_path = '/media/teerthal/Repo/Delayed_oscillation/%s'%(Data_set)
Master_path = '/str1/Teerthal/Reheating/%s'%(Data_set)
Master_path = "/media/teerthal/Repo 2/Reheating/Set_12"
...
targ_f = [0.1,0.05,0.01]
hel_sel_spool_args = [p, alpha, k, x_len, a_stack, Master_path, f, trigger_matrix, kill_matrix, Phi_stack, asymp_stack]
load_or_exec_multi_file(Master_path, 'Helicity', hel_spooler_sel, )
#exit()
