...
def rho_hel_plot(p, alpha, k, x_len, a_stack, Master_path, f, trigger_matrix, kill_matrix, Phi_Stack, asymp_stack):
    ...
    targ_alpha = [[2.2,2.6,3], [1.4,1.6,2]]
    # alpha_idxs=range(len(alpha))
    alpha_idxs =[[np.argmin(abs(targ - alpha)) for targ in sub_targ] for sub_targ in targ_alpha]
    f_idxs = [np.argmin(abs(targ - f)) for targ in targ_f]
    colors = cm.rainbow(np.linspace(0, 1, len(alpha_idxs[0])));
    ...
    for cnt_f, idx_f in enumerate(f_idxs):
        ...
        for cnt_p, idx_p, in enumerate(p_idxs):
            ...
            color_cnt = 0
            for cnt_alpha, idx_alpha in enumerate(alpha_idxs[idx_f]):
                ...
                print(r'$H_{\rm osc}/m:%.6f$'%H_map[id_max])
                print(r'$H_{\rm osc}/1e10:%.4f$'%(H_map[id_max]*m*Mpl/1e10))
                print(r'$H_{0}/1e10:%.4f$' % (H_map[0] * m * Mpl/1e10))
                print(r'$\rho_\phi_{osc}/1e14:%.4f$' % ((3 * H_map[id_max] ** 2 * m ** 2 * Mpl ** 4)**(1/4)/1e14))
                print(r'$\rho_\phi_i/1e14:%.4f$' % ((3 * H_map[0] ** 2 * m ** 2 * Mpl ** 4)**(1/4)/1e14))
                ...
                ratio = \
                (((rho_B_1 + rho_B_2) / a_map ** 4 + (rho_E_1 + rho_E_2) / a_map ** 2) / (3 * H_map ** 2*m**2*Mpl**4))[-1]
                print(r'$\alpha:$', item_alpha, 'f:', item_f, 'Ratio:%.5f' % ratio)
                a_R = T_0 / (T_R * Mpl)  # ; a_R = 1e-29  ## a_rT_r = a_0T_r
                ...
                ax0.plot([],[], label = r'$\alpha:%.1f$'%item_alpha, color=colors[color_cnt])

                color_cnt = color_cnt + 1

        #ax0.semilogy(x_B, 3 * H_map ** 2 / (m ** 2),
         #            color='k')
...
def rho_B_E_hel_plot(p, alpha, k, x_len, a_stack, Master_path, f, trigger_matrix, kill_matrix, Phi_Stack, asymp_stack):
    h = [-1, 1]
    ...
    for cnt_f, idx_f in enumerate(f_idxs):
        ...
        for cnt_p, idx_p, in enumerate(p_idxs):
            ...
            for cnt_alpha, idx_alpha in enumerate(alpha_idxs[idx_f]):
                ...
                if ratio < 2:
                    ...
                    print(r'$B_{0, peak}:%.4e$' % (B_0))
                    # a_R = 1e-29 ##Subhramanian entropy conservation
                cross_idx_B1 = np.argwhere(np.diff(np.sign(np.absolute(
                     rho_B_1 / a_map ** 4) - 3 * H_map ** 2 * m ** 2 * Mpl ** 4))).flatten()
                cross_idx_B2 = np.argwhere(np.diff(np.sign(np.absolute(
                    + rho_B_2 / a_map ** 4) - 3 * H_map ** 2 * m ** 2 * Mpl ** 4))).flatten()

                cross_idx_E1 = np.argwhere(np.diff(np.sign(np.absolute(
                    rho_E_1 / a_map ** 2 ) - 3 * H_map ** 2 * m ** 2 * Mpl ** 4))).flatten()
                cross_idx_E2 = np.argwhere(np.diff(np.sign(np.absolute(
                    rho_E_2 / a_map ** 2 ) - 3 * H_map ** 2 * m ** 2 * Mpl ** 4))).flatten()

                if len(cross_idx_B1) == 0:
                    cross_idx_B1 = -1
                    ax0.semilogy(x_B[:cross_idx_B1], (rho_B_1 / a_map ** 4 )[:cross_idx_B1],
                                 color=colors[color_cnt], linestyle=':')
                    ax0.semilogy(x_B[cross_idx_B1:], (rho_B_1 / a_map ** 4 )[cross_idx_B1:],
                                 color=colors[color_cnt], linestyle=':', alpha=0.5)

                    ax1.plot(x_hel[:cross_idx_B1], Hel[:cross_idx_B1], color=colors[color_cnt])
                    ax1.plot(x_hel[cross_idx_B1:], Hel[cross_idx_B1:], color=colors[color_cnt], alpha = 0.5, linestyle = '--')

                else:
                    cross_idx_B1 = cross_idx_B1[0]
                    ax0.semilogy(x_B[:cross_idx_B1 + 1], (rho_B_1 / a_map ** 4 )[:cross_idx_B1 + 1],
                                 color=colors[color_cnt], linestyle=':')
                    ax0.semilogy(x_B[cross_idx_B1:], (rho_B_1 / a_map ** 4 )[cross_idx_B1:],
                                 color=colors[color_cnt], linestyle=':', alpha=0.5)
                ...
                if len(cross_idx_B2) == 0:
                    cross_idx_B2 = -1
                    ax0.semilogy(x_B[:cross_idx_B2], (rho_B_2 / a_map ** 4 )[:cross_idx_B2],
                                 color=colors[color_cnt], linestyle='-')
                    ax0.semilogy(x_B[cross_idx_B2:], (rho_B_2 / a_map ** 4)[cross_idx_B2:],
                                 color=colors[color_cnt], linestyle='-', alpha=0.5)

                else:
                    cross_idx_B2 = cross_idx_B2[0]
                    ax0.semilogy(x_B[:cross_idx_B2 + 1], (rho_B_2 / a_map ** 4 )[:cross_idx_B2 + 1],
                                 color=colors[color_cnt], linestyle='-')
                    ax0.semilogy(x_B[cross_idx_B2:], (rho_B_2 / a_map ** 4)[cross_idx_B2:],
                                 color=colors[color_cnt], linestyle='-', alpha=0.5)
                ...
                if len(cross_idx_E1) == 0:
                    cross_idx_E1 = -1
                    ax0.semilogy(x_B[:cross_idx_E1], (rho_E_1 / a_map ** 2)[:cross_idx_E1],
                                 color=colors[color_cnt], linestyle='--')
                    ax0.semilogy(x_B[cross_idx_E1:], (rho_E_1 / a_map ** 2)[cross_idx_E1:],
                                 color=colors[color_cnt], linestyle='--', alpha=0.5)

                else:
                    cross_idx_E1 = cross_idx_E1[0]
                    ax0.semilogy(x_B[:cross_idx_E1 + 1], (rho_E_1 / a_map ** 2)[:cross_idx_E1 + 1],
                                 color=colors[color_cnt], linestyle='--')
                    ax0.semilogy(x_B[cross_idx_E1:], (rho_E_1 / a_map ** 2)[cross_idx_E1:],
                                 color=colors[color_cnt], linestyle='--', alpha=0.5)

                if len(cross_idx_E2) == 0:
                    cross_idx_E2 = -1
                    ax0.semilogy(x_B[:cross_idx_E2], (rho_E_2 / a_map ** 2)[:cross_idx_E2],
                                 color=colors[color_cnt], linestyle='-.')
                    ax0.semilogy(x_B[cross_idx_E2:], (rho_E_2 / a_map ** 2)[cross_idx_E2:],
                                 color=colors[color_cnt], linestyle='-.', alpha=0.5)
                    ...
                    cross_idx_E2 = cross_idx_E2[0]
                    ax0.semilogy(x_B[:cross_idx_E2 + 1], (rho_E_2 / a_map ** 2)[:cross_idx_E2 + 1],
                                 color=colors[color_cnt], linestyle='-.')
                    ax0.semilogy(x_B[cross_idx_E2:], (rho_E_2 / a_map ** 2)[cross_idx_E2:],
                                 color=colors[color_cnt], linestyle='-.', alpha=0.5)
        ...
        ax0.semilogy(x_B, 3 * H_map ** 2 * m ** 2 * Mpl ** 4,
                     color='k')  # ;plt.semilogy(t, 3*H_map_T**2/m**2,color='k', linestyle = '--')
        print(r'$\rho_\phi_i:%.2e$'%(H_map[0] ** 2 * m ** 2 * Mpl ** 4))
        ax0.set_ylim(1e50, 1e70)
        ax0.legend()
    ax0.plot([],[],linestyle = '--', label = r'$\rho_E^(2)$', color = 'k')
    ax0.plot([], [], linestyle='-', label = r'$\rho_B^(2)$', color = 'k')
    ax0.plot([], [], linestyle=':', label = r'$\rho_B^(1)$', color = 'k')
    ax0.plot([], [], linestyle='-.', label = r'$\rho_E^(1)i$', color = 'k')
    ax0.legend()
